...
async function main() {
  console.log("Seeding database with mock data for child_001...");

  // ─── ArtworkAnalysis records ────────────────────────────────────────────────
  ...
  console.log(`Created ${januaryArtworks.length + februaryArtworks.length + marchArtworks.length} ArtworkAnalysis records`);
  ...
  const insights = [
    ...
    },
    {
      ...
      sentiment: "Energetic",
      milestone_detected:
        "First self-portrait with full facial features! In her February 10th drawing, Zohar included eyes, nose, mouth, ears, and even hair — a significant leap in self-awareness and observational skill.",
  ...
  for (const insight of insights) {
    ...
    await prisma.insights.upsert({
      ...
      update: {
        ...
        encouragement_scripts: Array.isArray(encouragement_scripts)
